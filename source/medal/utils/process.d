...
// Copied from std.process in Phobos
    ...
    */
    ...
    void forkChild() nothrow @nogc
    {
        ...
        void execProcess()
        {
            ...
            if (sigfillset(&ss) != 0)
            {
                abortOnError(forkPipeOut, InternalError.sigfillset, .errno);
            }
            if (sigprocmask(SIG_UNBLOCK, &ss, null) != 0)
            {
                abortOnError(forkPipeOut, InternalError.sigprocmask, .errno);
            }
        ...
        if (status != InternalError.noerror)
        {
            ...
            final switch (status)
            {
                ...
                case InternalError.malloc:
                    ...
                    break;
                case InternalError.sigfillset:
                    errorMsg = "Failed to initialize signal set";
                    break;
                case InternalError.sigprocmask:
                    errorMsg = "Failed to set signal mask";
                    break;
